...
lines 500 ft apart, classify color, and log results.
...
YOLO_WEIGHTS       = "yolov8n.pt"   # use yolov8s.pt if your GPU is beefy
...
def get_dominant_color(roi: np.ndarray) -> str:
    """Return a coarse color name (Red/Green/Blue/…/Gray) for the ROI."""
    if roi.size == 0:
        return "Unknown"
...
def draw_boundaries(frame, line_pos1, line_pos2, orientation):
    """Draw horizontal or vertical lines on the frame."""
    h, w = frame.shape[:2]
...
def crossed_boundary(coord_prev, coord_curr, boundary_pos):
    """
    Return True if a coordinate moved from before to after the boundary.
    Works for both orientations (coord = x or y).
    """
...
def main():
    """
    with safe_globals([
        DetectionModel,  # Ultralytics model wrapper
        Conv,            # Custom conv layer used in YOLOv8
        Sequential,      # Standard PyTorch container
        Conv2d,          # Standard 2D conv layer
        BatchNorm2d,     # Standard batchnorm
        SiLU,             # Activation function used by YOLOv8
        C2F,
    ]):"""
    # Load YOLO model (will use GPU if available / CUDA)
    ...
    try:
        while True:
            while len(buffer) < HEADER:
                ...
                buffer += chunk
            ts, size = struct.unpack("dI", buffer[:HEADER])
            ...
            tracks = updated  # discard stale tracks
